# Lecture 04 - mlir-opt, mlir-translate, FileCheck

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `mlir-opt, mlir-translate, FileCheck`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_04_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_04_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_04_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_04_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_04_Quiz.md`
...
- `examples/01_parse_verify.mlir` - MLIR IR 예제
- `examples/02_cse_regression_test.mlir` - MLIR IR 예제
- `examples/03_pass_pipeline_debug.mlir` - MLIR IR 예제
- `examples/04_filecheck_directives.mlir` - MLIR IR 예제
- `examples/05_verify_diagnostics_negative.mlir` - MLIR IR 예제
- `examples/06_mlir_translate_to_llvm.mlir` - MLIR IR 예제
- `examples/07_npu_pass_test_template.mlir` - MLIR IR 예제
- `examples/lit_local.cfg.py` - 계산/분석 스크립트
- `examples/run_lesson04.sh` - 실습 실행 스크립트
...
```bash
cd lectures/lecture-04
bash examples/run_lesson04.sh
```
...
- `mlir-opt`를 아무 pass 없이 실행했을 때 얻는 가장 기본적인 효과는 무엇인가요?
- `--pass-pipeline='builtin.module(func.func(cse,canonicalize))'`에서 `builtin.module`과 `func.func`의 의미는 무엇인가요?
- NPU compiler pass test에서 `CHECK-NOT: stablehlo.dot_general`이 유용한 이유는 무엇인가요?
- `CHECK-DAG`를 써야 하는 경우와 쓰면 위험한 경우를 각각 설명하세요.
- `-verify-diagnostics` 기반 negative test가 필요한 NPU compiler 사례를 2개 드세요.
