# Lecture 12 - Lecture 12 - Bufferization: Tensor to MemRef, Alias, and Lifetime Analysis

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Lecture 12 - Bufferization: Tensor to MemRef, Alias, and Lifetime Analysis`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_12_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_12_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_12_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_12_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_12_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_12_Quick_Reference.md`
...
- `examples/01_tensor_vs_memref_boundary.mlir` - MLIR IR 예제
- `examples/02_linalg_on_tensors_before_bufferization.mlir` - MLIR IR 예제
- `examples/03_one_shot_bufferize_pipeline.mlir` - MLIR IR 예제
- `examples/04_bufferization_conflict_example.mlir` - MLIR IR 예제
- `examples/05_alloc_tensor_anchor.mlir` - MLIR IR 예제
- `examples/06_npu_memory_planning_pseudo.mlir` - MLIR IR 예제
- `examples/07_bufferizable_op_interface_skeleton.cpp` - C++/ODS skeleton
- `examples/08_filecheck_bufferization_test.mlir` - MLIR IR 예제
- `examples/09_lifetime_table_template.md` - 체크리스트/설명 문서
- `examples/run_lesson12.sh` - 실습 실행 스크립트
...
```bash
cd lectures/lecture-12
bash examples/run_lesson12.sh
```
...
- Bufferization primarily converts which semantic boundary?
