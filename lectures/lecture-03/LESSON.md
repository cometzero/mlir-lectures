# Lecture 03 - Region and Block: NPU kernel/command region pseudo IR

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Region and Block: NPU kernel/command region pseudo IR`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_03_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_03_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_03_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_03_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_03_Quiz.md`
...
- `examples/MLIR_NPU_Lecture_03_NPU_Command_Region.mlir` - MLIR IR 예제
- `examples/MLIR_NPU_Lecture_03_Region_Block_Examples.mlir` - MLIR IR 예제
...
cd lectures/lecture-03
../../scripts/run_lesson.sh 03 --try-mlir
```
...
- MLIR의 기본 중첩 구조를 `Operation`, `Region`, `Block` 순서로 설명하세요.
- Region의 의미를 최종적으로 결정하는 것은 region 자체인가요, parent operation인가요?
- MLIR에서 LLVM PHI node에 해당하는 merge value는 보통 무엇으로 표현되나요?
- `scf.for`의 induction variable은 어떤 형태로 loop body에 전달되나요?
- SSACFG region의 block 마지막에는 무엇이 필요하나요?
