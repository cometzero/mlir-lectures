# Lecture 17 - Fusion, Layout Propagation, Transformer MLP fusion candidate analysis

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Fusion, Layout Propagation, Transformer MLP fusion candidate analysis`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_17_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_17_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_17_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_17_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_17_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_17_Quick_Reference.md`
...
- `examples/01_transformer_mlp_before.mlir` - MLIR IR 예제
- `examples/02_mlp_fusion_after_pseudo.mlir` - MLIR IR 예제
- `examples/03_layout_propagation_pseudo.mlir` - MLIR IR 예제
- `examples/04_fusion_candidate_matrix.md` - 체크리스트/설명 문서
- `examples/05_fusion_pass_skeleton.cpp` - C++/ODS skeleton
- `examples/06_filecheck_fusion_layout_test.mlir` - MLIR IR 예제
- `examples/07_memory_traffic_calculator.py` - 계산/분석 스크립트
- `examples/08_npu_mlp_fusion_policy.yaml` - 정책/스펙 YAML
- `examples/09_transform_schedule_for_mlp.mlir` - MLIR IR 예제
- `examples/10_layout_propagation_checklist.md` - 체크리스트/설명 문서
...
cd lectures/lecture-17
../../scripts/run_lesson.sh 17 --try-mlir
```
...
- `MatMul + Bias + GELU` fusion이 항상 좋은 최적화가 아닌 이유를 3가지 쓰세요.
- Layout propagation과 layout materialization의 차이를 설명하세요.
- Transformer MLP에서 first GEMM output의 layout을 blocked로 유지하면 second GEMM에 어떤 장단점이 있나요?
- Per-channel quantization axis가 layout propagation에서 문제가 되는 사례를 쓰세요.
- 다음 중 FileCheck에 반드시 넣어야 할 항목은 무엇인가요? `fused op 존재`, `unfused op 부재`, `layout attr`, `materialization op count`.
