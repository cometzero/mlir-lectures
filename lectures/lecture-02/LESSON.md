# Lecture 02 - MLIR IR Basic Structure

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `MLIR IR Basic Structure`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_02_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_02_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_02_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_02_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_02_Quiz.md`
...
- `examples/MLIR_NPU_Lecture_02_IR_Examples.mlir` - MLIR IR 예제
...
cd lectures/lecture-02
../../scripts/run_lesson.sh 02 --try-mlir
```
...
- In MLIR, what is the difference between an operation result and a block argument?
- Why should a runtime pointer address not usually be modeled as an attribute?
- In `tensor<1x128x768xf16>`, which information is shape and which is element type?
- What is the benefit of generic operation assembly when debugging a custom NPU dialect?
- Name three NPU compiler decisions that can be represented as attributes.
