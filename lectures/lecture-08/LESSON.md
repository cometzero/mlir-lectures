...
- 슬라이드: `MLIR_NPU_Lecture_08_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_08_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_08_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_08_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_08_Quiz.md`
...
- `examples/01_scf_for_basic.mlir` - MLIR IR 예제
- `examples/02_scf_tiled_matmul_pseudo.mlir` - MLIR IR 예제
- `examples/03_affine_tiled_matmul.mlir` - MLIR IR 예제
- `examples/04_dma_double_buffer_pseudo.mlir` - MLIR IR 예제
- `examples/05_edge_tile_policy.mlir` - MLIR IR 예제
- `examples/06_filecheck_template.mlir` - MLIR IR 예제
- `examples/07_tile_size_sram_calculator.py` - 계산/분석 스크립트
- `examples/run_lesson08.sh` - 실습 실행 스크립트
...
```bash
cd lectures/lecture-08
bash examples/run_lesson08.sh
```
...
- `scf.for`의 lower bound, upper bound, step, induction variable은 각각 어떤 역할인가?
- `scf.for`의 `iter_args`와 `scf.yield`는 어떤 관계인가?
- Affine dialect가 SCF보다 loop analysis에 유리한 이유는 무엇인가?
- MatMul에서 `m0/n0/k0` tile loop의 일반적인 순서와 각 loop의 data reuse를 설명하라.
- `TM=32`, `TN=64`, `TK=64`, int8 input, int32 accumulator일 때 A/B/Accumulator tile 크기를 계산하라.
