...
- 이번 장의 핵심 주제인 `Transform Dialect`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_15_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_15_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_15_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_15_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_15_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_15_Quick_Reference.md`
...
- `examples/01_payload_linalg_matmul_add_relu.mlir` - MLIR IR 예제
- `examples/02_transform_schedule_tile_fuse_vectorize.mlir` - MLIR IR 예제
- `examples/03_two_level_tiling_schedule_pseudo.mlir` - MLIR IR 예제
- `examples/04_npu_schedule_spec.yaml` - 정책/스펙 YAML
- `examples/05_filecheck_transform_test.mlir` - MLIR IR 예제
- `examples/06_transform_interpreter_run.sh` - 실습 실행 스크립트
- `examples/07_custom_npu_transform_extension_skeleton.cpp` - C++/ODS skeleton
- `examples/08_schedule_review_checklist.md` - 체크리스트/설명 문서
- `examples/09_transform_debugging_notes.md` - 체크리스트/설명 문서
...
cd lectures/lecture-15
../../scripts/run_lesson.sh 15 --try-mlir
```
...
- Transform Dialect에서 payload IR과 transform IR의 차이를 설명하세요.
- `tile_using_for` 이후 원래 handle을 계속 쓰면 왜 위험한가요?
- NPU native tile이 16x16x32일 때 `vector.contract`의 lhs/rhs/acc vector shape를 제안하세요.
- Bias+ReLU epilogue fusion이 DRAM traffic을 줄이는 이유를 설명하세요.
- `silenceable failure`와 `definite failure`의 차이를 NPU fallback 관점에서 설명하세요.
