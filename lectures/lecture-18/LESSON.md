# Lecture 18 - Custom NPU Dialect: npu.matmul / npu.dma / npu.barrier ODS Design

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Custom NPU Dialect: npu.matmul / npu.dma / npu.barrier ODS Design`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_18_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_18_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_18_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_18_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_18_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_18_Quick_Reference.md`
...
- `examples/01_npu_dialect_overview.mlir` - MLIR IR 예제
- `examples/02_npu_ops_td_skeleton.td` - C++/ODS skeleton
- `examples/03_npu_types_attrs_td_skeleton.td` - C++/ODS skeleton
- `examples/04_npu_op_verifier_skeleton.cpp` - C++/ODS skeleton
- `examples/05_npu_parse_print_skeleton.cpp` - C++/ODS skeleton
- `examples/06_npu_matmul_dma_barrier_example.mlir` - MLIR IR 예제
- `examples/07_conversion_to_npu_kernel_skeleton.cpp` - C++/ODS skeleton
- `examples/08_filecheck_npu_dialect_test.mlir` - MLIR IR 예제
- `examples/09_negative_verifier_tests.mlir` - MLIR IR 예제
- `examples/10_cmake_skeleton.txt`
- `examples/11_npu_dialect_design_checklist.md` - 체크리스트/설명 문서
...
cd lectures/lecture-18
../../scripts/run_lesson.sh 18 --try-mlir
```
...
- ODS가 custom dialect 설계에서 single source of truth가 되는 이유는?
- `npu.matmul`에서 tile shape는 operand, attribute, type 중 무엇이 적합한가요?
- token-based `npu.dma` 모델의 장단점은?
- verifier가 reject해야 하는 조건과 cost model이 판단해야 하는 조건을 구분하세요.
- `npu.barrier`에 scope attribute가 필요한 이유는?
