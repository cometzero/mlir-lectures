...
- 이번 장의 핵심 주제인 `Runtime / ABI / Codegen`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_19_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_19_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_19_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_19_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_19_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_19_Quick_Reference.md`
...
- `examples/01_runtime_abi_overview.md` - 체크리스트/설명 문서
- `examples/02_command_buffer_schema.yaml` - 정책/스펙 YAML
- `examples/03_tensor_descriptor_abi.md` - 체크리스트/설명 문서
- `examples/04_npu_rt_dialect_pseudo.mlir` - MLIR IR 예제
- `examples/05_emit_command_buffer_pseudo.cpp` - C++/ODS skeleton
- `examples/06_mlir_to_llvm_pipeline.mlir` - MLIR IR 예제
- `examples/07_emitc_runtime_stub.mlir` - MLIR IR 예제
- `examples/08_filecheck_runtime_abi_test.mlir` - MLIR IR 예제
- `examples/09_runtime_launcher_c_api.h` - C++/ODS skeleton
- `examples/10_capability_and_fallback_policy.yaml` - 정책/스펙 YAML
- `examples/run_lesson19.sh` - 실습 실행 스크립트
...
```bash
cd lectures/lecture-19
bash examples/run_lesson19.sh
```
...
- Why is a tensor descriptor more than a pointer?
- What is the difference between logical shape and physical stride?
- Why should command buffers include an ABI version and target ID?
- Explain the difference between a command-internal barrier and a host-visible fence.
- When is EmitC a better codegen path than LLVM IR for edge deployment?
