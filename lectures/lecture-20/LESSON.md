# Lecture 20 - Lecture 20 - Capstone: Mini NPU Compiler Design Spec

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Lecture 20 - Capstone: Mini NPU Compiler Design Spec`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_20_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_20_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_20_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_20_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_20_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_20_Quick_Reference.md`
...
- `examples/01_capstone_brief.md` - 체크리스트/설명 문서
- `examples/02_mini_npu_compiler_design_spec_template.md` - 체크리스트/설명 문서
- `examples/03_pipeline_spec.yaml` - 정책/스펙 YAML
- `examples/04_npu_dialect_capstone_pseudo.mlir` - MLIR IR 예제
- `examples/05_transform_schedule_capstone.mlir` - MLIR IR 예제
- `examples/06_runtime_abi_manifest.yaml` - 정책/스펙 YAML
- `examples/07_filecheck_capstone_tests.mlir` - MLIR IR 예제
- `examples/08_cost_model_calculator.py` - 계산/분석 스크립트
- `examples/09_demo_script_template.md` - 체크리스트/설명 문서
- `examples/10_rubric_and_review_checklist.md` - 체크리스트/설명 문서
- `examples/11_presentation_outline.md` - 체크리스트/설명 문서
...
cd lectures/lecture-20
../../scripts/run_lesson.sh 20 --try-mlir
```
...
- Why is a legal op set more than a list of operation names?
- What information belongs in an operand vs an attribute for `npu.matmul`?
- Why should bufferization usually happen after tensor-level fusion and scheduling decisions?
- What are two negative verifier tests for a custom NPU dialect?
- What fields must be present in a runtime tensor descriptor?
