# Lecture 06 - StableHLO, TOSA, and AI Model Input IR

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `StableHLO, TOSA, and AI Model Input IR`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_06_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_06_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_06_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_06_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_06_Quiz.md`
...
- `examples/MLIR_NPU_Lecture_06_Decomposition_Checklist.md` - 체크리스트/설명 문서
- `examples/MLIR_NPU_Lecture_06_Runtime_Import_Notes.md` - 체크리스트/설명 문서
- `examples/MLIR_NPU_Lecture_06_StableHLO_Examples.mlir` - MLIR IR 예제
- `examples/MLIR_NPU_Lecture_06_TOSA_Examples.mlir` - MLIR IR 예제
...
../../scripts/run_lesson.sh 06 --try-mlir
```
...
- StableHLO가 NPU compiler input IR로 유용한 이유를 한 문장으로 설명하세요.
- TOSA가 hardware target concern을 고려한다는 말이 NPU compiler 설계에서 어떤 의미인지 설명하세요.
- LayerNorm을 primitive ops로 분해할 때 필요한 reduction은 몇 번인가요?
- LayerNorm에서 epsilon은 어느 단계에 들어가야 하나요?
- GELU exact 수식과 tanh approximation 수식의 차이를 설명하세요.
