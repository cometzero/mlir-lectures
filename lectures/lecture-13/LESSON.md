...
- 이번 장의 핵심 주제인 `MemRef, Layout, Memory Space`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_13_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_13_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_13_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_13_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_13_Quiz.md`
- Quick Reference: `MLIR_NPU_Lecture_13_Quick_Reference.md`
...
- `examples/01_memref_type_examples.mlir` - MLIR IR 예제
- `examples/02_subview_tile_buffers.mlir` - MLIR IR 예제
- `examples/03_memory_space_mapping.mlir` - MLIR IR 예제
- `examples/04_dma_memory_space_pseudo.mlir` - MLIR IR 예제
- `examples/05_memref_descriptor_abi.md` - 체크리스트/설명 문서
- `examples/06_memory_space_pass_skeleton.cpp` - C++/ODS skeleton
- `examples/07_filecheck_memref_layout_test.mlir` - MLIR IR 예제
- `examples/08_sram_footprint_calculator.py` - 계산/분석 스크립트
...
cd lectures/lecture-13
../../scripts/run_lesson.sh 13 --try-mlir
```
...
- `memref<32x?xf16, strided<[?,1], offset:?>, 1>`에서 runtime descriptor로 남아야 하는 metadata를 쓰세요.
- `memref.subview`가 copy가 아니라 view인 이유를 offsets/sizes/strides 관점에서 설명하세요.
- memory space를 raw integer보다 custom attribute로 표현하면 좋은 이유를 세 가지 쓰세요.
- `A_tile[64x128xf16]`, `B_tile[128x64xf16]`, `C_acc[64x64xi32]`의 byte footprint를 계산하세요.
- `memref.memory_space_cast`와 DMA copy의 차이를 설명하세요.
