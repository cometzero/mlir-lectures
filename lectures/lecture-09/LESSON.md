# Lecture 09 - Pass Infrastructure

이 파일은 원본 자료를 강의자가 바로 진행할 수 있도록 재구성한 진행안입니다. 원본 PDF/PPTX/DOCX는 보존되어 있고, 실습은 `examples/`와 `Worksheet`를 기준으로 진행합니다.

## 1. 학습 목표

- 이번 장의 핵심 주제인 `Pass Infrastructure`를 MLIR/NPU compiler pipeline 안에서 설명할 수 있습니다.
...
- 슬라이드: `MLIR_NPU_Lecture_09_Slides.pdf`
- 강의 노트: `MLIR_NPU_Lecture_09_Notes.pdf`
- 핸드아웃: `MLIR_NPU_Lecture_09_Handout.md`
- 워크시트: `MLIR_NPU_Lecture_09_Worksheet.pdf`
- 퀴즈: `MLIR_NPU_Lecture_09_Quiz.md`
...
- `examples/01_pass_pipeline_input.mlir` - MLIR IR 예제
- `examples/02_npu_lowering_pipeline_draft.mlir` - MLIR IR 예제
- `examples/03_npu_pass_skeleton.cpp` - C++/ODS skeleton
- `examples/04_npu_passes_td.td` - C++/ODS skeleton
- `examples/05_npu_pipeline_registration.cpp` - C++/ODS skeleton
- `examples/06_analysis_and_preservation.cpp` - C++/ODS skeleton
- `examples/07_filecheck_npu_pass_test.mlir` - MLIR IR 예제
- `examples/08_negative_diagnostic_test.mlir` - MLIR IR 예제
- `examples/run_lesson09.sh` - 실습 실행 스크립트
...
```bash
cd lectures/lecture-09
bash examples/run_lesson09.sh
```
...
- MLIR PassManager가 pass를 operation nesting level별로 schedule하는 이유를 두 가지 쓰세요.
- `OperationPass<func::FuncOp>`와 `OperationPass<>`의 차이를 설명하세요.
- Pass가 `npu_graph.matmul` op를 생성한다면 어떤 메서드에서 어떤 정보를 등록해야 하나요?
- `signalPassFailure()`가 호출되면 pipeline은 어떻게 동작하나요?
- `--mlir-print-ir-after-change`가 useful한 상황을 설명하세요.
